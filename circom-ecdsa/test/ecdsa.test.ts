...
describe("ECDSARecover", function () {
    ...
    let circuit: any;
    before(async function () {
        const output = path.join(__dirname, "compiled", "test_ecdsa_recover_dir");
        const compile = true;
        circuit = await wasm_tester(path.join(__dirname, "circuits", "test_ecdsa_recover.circom"), { output, compile });
    });
