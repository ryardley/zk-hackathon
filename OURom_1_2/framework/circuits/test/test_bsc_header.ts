...
function hexStringToBigIntArray(hexString: string, len: number) {
    ...
    for (let i = 0; i < hexString.length; i++) {
        current = current * 16n + BigInt(parseInt(hexString[i], 16))
        // current += BigInt(parseInt(hexString[i], 16)) * (16n ** BigInt(i % hexNum))
        if ((i + 1) % hexNum == 0) {
            result.push(current);
            current = 0n;
        }
...
function uint4ArrayToHexString(uint4Array: any) {
    ...
    for (let i = 0; i < uint4Array.length; i++) {
        const hex = uint4Array[i].toString(8);
        hexString += hex;
    }
    return hexString;
}
